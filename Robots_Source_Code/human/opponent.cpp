...
void Opponent::update(tSituation *s, tCarElt *mycar)
{
	...
	if (distance > -BACKCOLLDIST && distance < FRONTCOLLDIST) {
		...
		if (distance > SIDECOLLDIST && speed < mycar->getSpeed()) {
			catchdist = Opponent::getSpeed(mycar)*distance/(Opponent::getSpeed(mycar) - speed);
			state |= OPP_FRONT;
		...
		if (distance < -SIDECOLLDIST && speed > Opponent::getSpeed(mycar)) {
			catchdist = Opponent::getSpeed(mycar)*distance/(speed - Opponent::getSpeed(mycar));
			state |= OPP_BACK;
			distance -= MAX(car->_dimension_x, mycar->_dimension_x);
			distance -= LENGTH_MARGIN;
		} else
...
/* initialize the list of opponents */
Opponents::Opponents(tSituation *s, tCarElt *car, tTrack *track)
{
...
void Opponents::update(tSituation *s, tCarElt *car)
{
	int i;
	for (i = 0; i < s->_ncars - 1; i++) {
		opponent[i].update(s, car);
	}
}
